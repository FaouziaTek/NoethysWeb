...
from fiche_individu.utils import utils_internet
...
def ReinitTousMdp():
    for individu in Famille.objects.select_related("utilisateur").all():
...
def Purge_mdp_expires():
    ...
    utilisateurs = Utilisateur.objects.select_related("famille").filter(categorie="individu", date_expiration_mdp__lte=datetime.datetime.now() - datetime.timedelta(days=3))
...
def Fix_dates_expiration_mdp():
    ...
    for individu in Famille.objects.select_related("utilisateur").filter(utilisateur__date_expiration_mdp__isnull=True):
